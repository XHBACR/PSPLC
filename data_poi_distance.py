...
city = 'Gow'
df_poi_mapping = pd.read_csv(f"./my_data/raw/{city}_poi_mapping.csv")

# Get latitude and longitude
# latitude = df_poi_mapping['latitude'].values
# longitude = df_poi_mapping['longitude'].values
latitude = df_poi_mapping['latitude'].values.astype(np.float32)
longitude = df_poi_mapping['longitude'].values.astype(np.float32)
...
df_distance.to_csv(f'./my_data/raw/{city}_poi_distance.csv', index=False)

...
def generate_data(city):
    ...
    data = pd.read_csv(f"./my_data/raw/{city}_checkins.csv")
    ...
    valid_input_index = filter_long_short_term_sequences(total_sequences_meta, min_short_term_len, pre_seq_window,
                                                             min_long_term_count) #(data, 5,7,2) Filtered 384 valid input long+short term sequences for 43 users.
    ...
    # Initialize the lists to store the checkin sequences and the last checkins
    checkin_sequences = []
    last_checkins = []

    # Iterate over the train_data
    for i, sample in enumerate(train_data):
        # Concatenate the checkin sequences of each day
        checkin_sequence = [day[0] for day in sample]
        checkin_sequence = [item for sublist in checkin_sequence for item in sublist]
        for poi in checkin_sequence[:-1]:
            checkin_sequences.append((i, poi, 1))
        # checkin_sequences.append((i, checkin_sequence[:-1], 1))
        
        # Store the last checkin
        last_checkins.append((i, checkin_sequence[-1], 1))

    # Write the checkin sequences to the first txt file
    with open('train.txt', 'w') as f:
        for checkin_sequence in checkin_sequences:
            f.write(' '.join(map(str, checkin_sequence)) + '\n')

    # Write the last checkins to the second txt file
    with open('tune.txt', 'w') as f:
        for last_checkin in last_checkins:
            f.write(' '.join(map(str, last_checkin)) + '\n')
